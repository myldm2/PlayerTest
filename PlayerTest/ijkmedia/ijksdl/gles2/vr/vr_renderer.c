...
void makeSphereVertices(IJK_GLES2_Renderer *renderer) {
//    float altitude;
//    float altitudeDelta;
//    float azimuth;
//    float ex;
...
//    float ez;
//    int divide = 12;
////    GLfloat* vertices = renderer->vr_vertices;
////    GLfloat* texCoords = renderer->vr_texcoords;
//    GLfloat radius = 5;
//    for(int i = 0; i <= divide; i++) {
//
//        altitude      = (GLfloat) (M_PI/2.0 -    i    * (M_PI) / divide);
//        altitudeDelta = (GLfloat) (M_PI/2.0 - (i + 1) * (M_PI) / divide);
//        //创建纹理坐标点数组
//        for(int j = 0; j <= divide; j++) {
//            azimuth = (GLfloat)(j * (M_PI*2) / divide);
//            ex = (GLfloat) (cos(altitude) * cos(azimuth));
//            ey = (GLfloat)  sin(altitude);
//            ez = (GLfloat) - (cos(altitude) * sin(azimuth));
//            renderer->vr_vertices[6*j+0] = radius * ex;
//            renderer->vr_vertices[6*j+1] = radius * ey;
//            renderer->vr_vertices[6*j+2] = radius * ez;
//            //计算azimuth经度下纬度为altitude的纹理点坐标
//            renderer->vr_texcoords[4*j+0] = j/(GLfloat)divide;
//            renderer->vr_texcoords[4*j+1] = i/(GLfloat)divide;
//            ex = (GLfloat) (cos(altitudeDelta) * cos(azimuth));
//            ey = (GLfloat) sin(altitudeDelta);
//            ez = (GLfloat) -(cos(altitudeDelta) * sin(azimuth));
//            renderer->vr_vertices[6*j+3] = radius * ex;
//            renderer->vr_vertices[6*j+4] = radius * ey;
//            renderer->vr_vertices[6*j+5] = radius * ez;
//            //计算azimuth经度下纬度为altitudeDelta的纹理点坐标
//            renderer->vr_texcoords[4*j+2] = j/(GLfloat)divide;
//            renderer->vr_texcoords[4*j+3] = (i + 1) / (GLfloat)divide;
//        }
...
//    gluLookAt(2.0f,0.0f,4.0f,2.0f,0.0f,0.0f,2.0f,4.0f,0.0f);
    ...
    renderer->vr_vertices[9] =  1.0f;
    renderer->vr_vertices[10] =  1.0f;
    renderer->vr_vertices[11] = 1.0f;  //
    
    renderer->vr_texcoords[0] = 0.0f;
    renderer->vr_texcoords[1] = 1.0f;
    renderer->vr_texcoords[2] = 1.0f;
    renderer->vr_texcoords[3] = 1.0f;
    renderer->vr_texcoords[4] = 0.0f;
    renderer->vr_texcoords[5] = 0.0f;
    renderer->vr_texcoords[6] = 1.0f;
    renderer->vr_texcoords[7] = 0.0f;
}

static void IJK_GLES2_VR_printProgramInfo(GLuint program)
{
    if (!program)
        return;
...
void IJK_GLES2_VR_Renderer_reset(IJK_GLES2_Renderer *renderer)
{
    if (!renderer)
        return;
...
void IJK_GLES2_VR_Renderer_free(IJK_GLES2_Renderer *renderer)
{
    if (!renderer)
        return;
    ...
    for (int i = 0; i < IJK_GLES2_VR_MAX_PLANE; ++i) {
        if (renderer->plane_textures[i])
...
void IJK_GLES2_VR_Renderer_freeP(IJK_GLES2_Renderer **renderer)
{
    if (!renderer || !*renderer)
        return;

    IJK_GLES2_VR_Renderer_free(*renderer);
    *renderer = NULL;
}

IJK_GLES2_Renderer *IJK_GLES2_VR_Renderer_create_base(const char *fragment_shader_source)
{
...
fail:
    ...
    if (renderer && renderer->program)
        IJK_GLES2_VR_printProgramInfo(renderer->program);
...
IJK_GLES2_Renderer *IJK_GLES2_VR_Renderer_create(SDL_VoutOverlay *overlay)
{
    if (!overlay)
        return NULL;
...
GLboolean IJK_GLES2_VR_Renderer_isValid(IJK_GLES2_Renderer *renderer)
{
    return renderer && renderer->program ? GL_TRUE : GL_FALSE;
}

GLboolean IJK_GLES2_VR_Renderer_isFormat(IJK_GLES2_Renderer *renderer, int format)
{
    if (!IJK_GLES2_VR_Renderer_isValid(renderer))
        return GL_FALSE;
 ...
 * Per-Context routine
 */
GLboolean IJK_GLES2_VR_Renderer_setupGLES()
{
...
static void IJK_GLES2_VR_Renderer_Vertices_reset(IJK_GLES2_Renderer *renderer)
{
    ...
    makeSphereVertices(renderer);
}

static void IJK_GLES2_VR_Renderer_Vertices_apply(IJK_GLES2_Renderer *renderer)
{
    switch (renderer->gravity) {
        ...
        case IJK_GLES2_GRAVITY_RESIZE:
            IJK_GLES2_VR_Renderer_Vertices_reset(renderer);
            return;
        default:
...
static void IJK_GLES2_VR_Renderer_Vertices_reloadVertex(IJK_GLES2_Renderer *renderer)
{
    glVertexAttribPointer(renderer->av4_position, 3, GL_FLOAT, GL_FALSE, 0, renderer->vr_vertices);    IJK_GLES2_checkError_TRACE("glVertexAttribPointer(av2_texcoord)");
    glEnableVertexAttribArray(renderer->av4_position);                                      IJK_GLES2_checkError_TRACE("glEnableVertexAttribArray(av2_texcoord)");
}

#define IJK_GLES2_VR_GRAVITY_MIN                   (0)
#define IJK_GLES2_VR_GRAVITY_RESIZE                (0) // Stretch to fill layer bounds.
#define IJK_GLES2_VR_GRAVITY_RESIZE_ASPECT         (1) // Preserve aspect ratio; fit within layer bounds.
#define IJK_GLES2_VR_GRAVITY_RESIZE_ASPECT_FILL    (2) // Preserve aspect ratio; fill layer bounds.
#define IJK_GLES2_VR_GRAVITY_MAX                   (2)

GLboolean IJK_GLES2_VR_Renderer_setGravity(IJK_GLES2_Renderer *renderer, int gravity, GLsizei layer_width, GLsizei layer_height)
{
    if (renderer->gravity != gravity && gravity >= IJK_GLES2_VR_GRAVITY_MIN && gravity <= IJK_GLES2_VR_GRAVITY_MAX)
...
static void IJK_GLES2_VR_Renderer_TexCoords_reset(IJK_GLES2_Renderer *renderer)
{
...
static void IJK_GLES2_VR_Renderer_TexCoords_cropRight(IJK_GLES2_Renderer *renderer, GLfloat cropRight)
{
    ALOGE("IJK_GLES2_VR_Renderer_TexCoords_cropRight\n");
    renderer->texcoords[0] = 0.0f;
    renderer->texcoords[1] = 1.0f;
...
static void IJK_GLES2_VR_Renderer_TexCoords_reloadVertex(IJK_GLES2_Renderer *renderer)
{
    glVertexAttribPointer(renderer->av2_texcoord, 2, GL_FLOAT, GL_FALSE, 0, renderer->vr_texcoords);   IJK_GLES2_checkError_TRACE("glVertexAttribPointer(av2_texcoord)");
 ...
 */
GLboolean IJK_GLES2_VR_Renderer_use(IJK_GLES2_Renderer *renderer)
{
    if (!renderer)
        return GL_FALSE;
    ...
    IJK_GLES2_VR_Renderer_TexCoords_reset(renderer);
    IJK_GLES2_VR_Renderer_TexCoords_reloadVertex(renderer);

    IJK_GLES2_VR_Renderer_Vertices_reset(renderer);
    IJK_GLES2_VR_Renderer_Vertices_reloadVertex(renderer);
 ...
 * Per-Frame routine
 */
GLboolean IJK_GLES2_VR_Renderer_renderOverlay(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    if (!renderer || !renderer->func_uploadTexture)
        return GL_FALSE;

//    glClear(GL_COLOR_BUFFER_BIT);               IJK_GLES2_VR_checkError_TRACE("glClear");
    ...
    if (renderer->vertices_changed ||
        ...
        IJK_GLES2_VR_Renderer_Vertices_apply(renderer);
        IJK_GLES2_VR_Renderer_Vertices_reloadVertex(renderer);
        ...
        IJK_GLES2_VR_Renderer_TexCoords_reset(renderer);
        IJK_GLES2_VR_Renderer_TexCoords_cropRight(renderer, padding_normalized);
        IJK_GLES2_VR_Renderer_TexCoords_reloadVertex(renderer);
    }
