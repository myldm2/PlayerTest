...
function pull_fork() {
    ...
    sh $TOOLS/pull-repo-ref.sh $IJK_FFMPEG_FORK ffmpeg/ffmpeg-$1 ${IJK_FFMPEG_LOCAL_REPO}
    cd ffmpeg/ffmpeg-$1
...
function sync_ff_version() {
    sed -i '' "s/static const char \*kIJKFFRequiredFFmpegVersion\ \=\ .*/static const char *kIJKFFRequiredFFmpegVersion = \"${IJK_FFMPEG_COMMIT}\";/g" PlayerTest/IJKFFMoviePlayerController/IJKFFMoviePlayerController.m
}
